...
const WeatherWidget = ({ data }) => {
  const result = data.output?.result || data.output || data.result || data;
  if (!result) return null;

  const rawTemp = result.temperature !== undefined ? result.temperature : result.temp;
  const rawHumidity = result.humidity !== undefined ? result.humidity : result.humidity_percentage;
  const rawRain = result.rain_probability !== undefined ? result.rain_probability : (result.rainfall_probability !== undefined ? (result.rainfall_probability <= 1 ? result.rainfall_probability * 100 : result.rainfall_probability) : undefined);
  const location = result.location || "Local Area";

  const tempDisplay = rawTemp !== undefined ? `${Math.round(rawTemp)}°C` : "--°C";
  const humidityDisplay = rawHumidity !== undefined ? `${Math.round(rawHumidity)}%` : "--%";
  const rainDisplay = rawRain !== undefined ? `${Math.round(rawRain)}%` : "--%";
    ...
    <div className="mt-3 p-4 rounded-2xl bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-850 flex flex-col gap-2">
      ...
      <div className="mt-1">
        <span className="text-xs uppercase bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 px-2.5 py-1 rounded-full font-bold">
          {result.diagnosis} ({Math.round(result.confidence * 100)}% confidence)
        </span>
      </div>
  ...
  return (
    <div className="mt-3 p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/15 border border-amber-200 dark:border-amber-850 flex flex-col gap-2">
      <div className="flex items-center gap-2 text-amber-700 dark:text-amber-400 font-semibold">
        ...
        <h4>Market Valuation ({result.crop})</h4>
      </div>
  ...
  const renderToolLog = (log) => {
    switch (log.tool_name) {
      case 'weather_advisor':
        return <WeatherWidget key={log.tool_name} data={log} />;
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {isBot && message.tool_calls && message.tool_calls.length > 0 && (
          <div className="mt-1 space-y-2">
            {message.tool_calls.map(renderToolLog)}
          </div>
        )}
