...
from app.ai.llm import get_llm, GeminiLLM, MockQwenLLM
from app.ai.agent_executor import agent_brain
from app.core.config import settings

async def main():
    print("==================================================")
    print("    AGRIGENIUS GEMINI AI REAL-TIME AGENT VERIFICATION")
    print("==================================================")
    ...
    if isinstance(llm, GeminiLLM):
        print(f"✓ Gemini API Key Detected: {llm.get_effective_key()[:8]}...")
        print(f"✓ Gemini Model Target: {llm.get_effective_model()}")
    else:
        print(f"ℹ No valid GEMINI_API_KEY detected in env yet. Using active engine: {llm_type}")
